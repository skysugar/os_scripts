...
#-----------cut history log-----------------------------#
export PROMPT_COMMAND='{ msg=$(history 1 | { read x y; echo $y; });user=$(whoami); echo $(date "+%Y-%m-%d %H:%M:%S"):$user:`pwd`/:$msg ---- $(who am i); } >> /tmp/`host
name`.`whoami`.history-timestamp'
#-------------------------------------------------------#
